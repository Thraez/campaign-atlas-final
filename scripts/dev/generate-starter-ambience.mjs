...
function onePoleLP(alpha) {
  let y = 0;
  return (x) => (y += alpha * (x - y));
}
function onePoleHP(alpha) {
  const lp = onePoleLP(alpha);
  return (x) => x - lp(x);
}
...
// --- wind-hollow: brown noise, slowly wandering lowpass + gusting gain ---
{
  ...
  for (let i = 0; i < buf.length; i++) {
    ...
    const gust =
      0.55 +
      0.3 * Math.sin(2 * Math.PI * 0.05 * t) +
      0.15 * Math.sin(2 * Math.PI * 0.113 * t + 1.7);
    buf[i] = smooth(brown) * gust * 3;
  }
  const l = loopify(buf);
  normalize(l);
  writeWav(join(OUT, "wind-hollow.wav"), l);
}
...
// --- water-trickle: bandpassed noise with layered burble modulation ---
{
  ...
  for (let i = 0; i < buf.length; i++) {
    ...
    const burble =
      0.5 +
      0.2 * Math.sin(2 * Math.PI * 0.9 * t) * Math.sin(2 * Math.PI * 0.23 * t) +
      0.18 * Math.sin(2 * Math.PI * 1.7 * t + 0.9) +
      0.12 * Math.sin(2 * Math.PI * 0.41 * t + 2.1);
    buf[i] = hp(lp(rnd() * 2 - 1)) * burble;
  }
  const l = loopify(buf);
  normalize(l, 0.6);
  writeWav(join(OUT, "water-trickle.wav"), l);
}
...
// --- cavern-drone: detuned low sine cluster with slow beating + faint air ---
{
  ...
  const l = loopify(buf, 5);
  normalize(l, 0.55);
  writeWav(join(OUT, "cavern-drone.wav"), l);
}
console.log("done");
