    ...
    if (sc) {
      ...
      for (const a of areas) {
        ...
        if (typeof a.id !== "string" || !/^area-\d+$/.test(a.id)) {
          violations.push({
            field: `map[${mapId}].soundscape.area[${aId}].id`,
            message: `soundscape area id "${a.id}" is not neutralised (expected area-N)`,
          });
        }
        const vis = a.visibility as string | undefined;
        if (vis !== undefined && !PLAYER_VISIBLE.has(vis as never)) {
          violations.push({
            field: `map[${mapId}].soundscape.area[${aId}].visibility`,
            message: `soundscape area visibility "${vis}" not in PLAYER_VISIBLE`,
          });
        }
        if (a.name !== undefined) {
          violations.push({
            field: `map[${mapId}].soundscape.area[${aId}].name`,
            message: `soundscape area name "${a.name}" must be stripped in player builds`,
          });
        }
      }
    }
