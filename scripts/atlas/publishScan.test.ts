...
describe("publishScan reason mapping (D8: never echo secrets)", () => {
  it("maps a DM-content hit to a scan-level message with no secret", () => {
    const reasons = reasonsFromNoSecrets(
      {
        files: 1,
        dmHits: [{ file: "dist/x.js", pattern: "SENTINEL_DM_BODY_001", kind: "dm" }],
        editorHits: [],
      },
      "dist",
    );
    ...
    const reasons = reasonsFromNoSecrets(
      {
        files: 1,
        dmHits: [],
        editorHits: [{ file: "dist/x.js", pattern: "/__atlas/save", kind: "editor" }],
      },
      "dist",
    );
    ...
    const reasons = reasonsFromDerived(
      {
        ...
        hits: [
          {
            file: "dist/atlas/atlas.json",
            match: { name: SECRET, source: "content/world/cabal.md", field: "title" },
          },
        ],
      },
      "dist",
  ...
  it("shape: uses entityId as locator and NEVER the violation message", () => {
    ...
    const reasons = reasonsFromShape({
      violations: [
        { entityId: "the-villain", field: "sourcePath", message: `sourcePath leaked: "${LEAK}"` },
      ],
    });
...
describe("publishScan dedup + fallback behavior", () => {
    ...
    const reasons = reasonsFromDerived(
      {
        ...
        hits: [
          {
            file: "dist/atlas/atlas.json",
            match: { name: "Secret One", source: "content/world/cabal.md", field: "title" },
          },
          {
            file: "dist/atlas/atlas.json",
            match: { name: "Secret Two", source: "content/world/cabal.md", field: "alias" },
          },
        ],
      },
      "dist",
    ...
    const reasons = reasonsFromDerived(
      {
        ...
        hits: [
          ...
          {
            file: "dist/atlas/atlas.json",
            match: { name: "Secret Two", source: "content/world/lich.md", field: "title" },
          },
        ],
      },
      "dist",
  ...
  it("shape: two violations on the SAME entity collapse to one reason", () => {
    const reasons = reasonsFromShape({
      violations: [
        {
          entityId: "the-villain",
          field: "sourcePath",
          message: 'sourcePath leaked: "spoiler.md"',
        },
