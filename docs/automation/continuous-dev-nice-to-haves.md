...
- [x] **N132. Wire `aria-controls`/`aria-activedescendant` on the body editor's wikilink autocomplete.** ✅ DONE 2026-07-29 — commit 886c1e70
  Confirmed: the `[[`/`![[` autocomplete textarea in `EntityEditPanel.tsx` set `aria-autocomplete="list"` and `aria-expanded` but the `WikilinkPopover` it opens carried no id and none of its options did either, so the textarea had nothing to point `aria-controls`/`aria-activedescendant` at (cited lines had drifted from `:437-461` to `:445-470`, premise otherwise held). Gave the popover's listbox div a stable `id="wikilink-popover-listbox"` and each option an index-based `id={`wikilink-option-${i}`}` (index-based rather than value-based since image filenames can contain characters unsafe for bare use as ids). `EntityEditPanel` now computes the same clamped active index the popover uses internally (`Math.min(acIndex, Math.max(0, itemCount - 1))`) and sets the textarea's `aria-controls` to the listbox id whenever the popover is open, and `aria-activedescendant` to the active option's id whenever there's at least one match (omitted — not just pointed at a nonexistent id — when the popover shows "No matches"). 2 new tests in `src/test/categories/EntityEditPanel.test.tsx` (`describe("wikilink autocomplete ARIA wiring (N132)")`): opening the image-autocomplete popover wires `aria-controls`/`aria-activedescendant` to the first option and ArrowDown moves both to the second; the no-matches case keeps `aria-controls` but omits `aria-activedescendant`. Gate: typecheck clean · lint 0 errors (18 pre-existing, unchanged) · 2935 tests green across the 4 shards (729+637+849+720 — +2 over the N131 baseline of 2933, matching the 2 new tests). One shard hit the documented `onTaskUpdate` RPC flake (0 real failures, not re-run per policy). Pure client-side component + test change — no `atlas:publish` required.

- [x] **N133. Make the image picker's delete button reachable by keyboard.** ✅ DONE 2026-07-29 — commit 27212f30
  Confirmed: the per-image delete button used `hidden group-hover:flex` (`src/atlas/editor/ImagePickerPanel.tsx:99`, cited range `:94-107` still matched) — `display:none` except on mouse hover, which meant it could never receive keyboard focus in the first place (a `display:none` element is never in the tab order, so a `group-focus-within` reveal could never trigger — Tab can't focus something that isn't rendered). Fixed by switching the button to always-rendered, toggling visibility/interactivity via `opacity-0 pointer-events-none` → `group-hover:opacity-100 group-hover:pointer-events-auto` and `group-focus-within:opacity-100 group-focus-within:pointer-events-auto`, so Tab reaches the button and it becomes visible + clickable when it (or the group) has focus; pointer hover behavior unchanged. 3 new tests in `src/test/editor/ImagePickerPanel.test.tsx` — button stays in the tab order and can receive `.focus()` (not `display:none`), the opacity/pointer-events classes are wired for both hover and focus-within, and no delete button renders when `onDelete` is omitted. Gate: typecheck clean · lint 0 errors (18 pre-existing, unchanged) · 2938 tests green across the 4 shards (729+637+849+723 — +3 over the N132 baseline of 2935, matching the 3 new tests). One shard hit the documented `onTaskUpdate` RPC flake (0 real failures, not re-run per policy). Pure client-side component + test change — no `atlas:publish` required.

- [x] **N134. Wire route midpoint markers to the already-built `insertWaypointAfter` mutation.** ✅ DONE 2026-07-29 — commit 4748e7ec
  Confirmed: `insertWaypointAfter` was fully implemented and exported by `useRouteDraft` (`src/atlas/routes/useRouteDraft.ts:229-238`, cited lines had drifted from `:255-264,51,415` but the gap held — a `grep` across `src/` found the mutation referenced only in its own hook file and tests, nothing in the UI) but nothing called it, so a DM couldn't add a midpoint to an existing route segment short of deleting and redrawing. Added a small dashed-outline `Marker` at the midpoint of each segment of the selected route in `RouteLayer.tsx` (new `midpointIcon`, className `atlas-route-midpoint-handle`, distinct from the existing solid waypoint-handle icon) — clicking one calls `api.insertWaypointAfter(selected.id, i, [midX, midY])`, splitting the segment in two. A segment is skipped (no marker) if either endpoint is an unresolved entity-ref waypoint, matching how the existing waypoint handles already skip unresolved points. Updated the Routes panel's help hint in `RoutesTab.tsx` to mention the new affordance.
  **Tests:** 7 new tests in `src/test/routes/RouteLayer.test.tsx` (new file) — one midpoint marker renders per segment; a marker's map position is the segment's true geometric midpoint; clicking a marker calls `insertWaypointAfter` with the correct route id, index, and rounded midpoint coords (checked for both the first and second segment); no midpoint markers when no route is selected; none while drawing a new route; a segment touching an unresolved entity-ref waypoint renders no marker. Uses a custom `Marker` mock (same pattern as `placement-markers.test.tsx`) exposing the divIcon's `className` and wiring `eventHandlers.click` to a real DOM click, plus a hand-built mock `RouteDraftAPI` (same shape as `RoutesTab.test.tsx`'s).
  **Gate:** typecheck clean · lint 0 errors (18 pre-existing, unchanged) · 2945 tests green across the 4 shards (736+640+865+704 — +7 over the N133 baseline of 2938, matching the 7 new tests). One shard hit the documented `onTaskUpdate` RPC flake (0 real failures, not re-run per policy). Pure client-side editor component + test change — no `scripts/`-only edit, no fog/soundscape touch, no shipped-artifact shape change — so `atlas:publish` wasn't required.
  **Side-effect (recurring, same as N107–N133):** the sharded vitest run's build-pipeline tests deleted the same 6 content-hashed audio files under `public/atlas/assets/audio/` in the run worktree. Caught via `git status` before the commit; reverted with `git checkout --` — no unrelated artifact churn landed on `auto/continuous-dev`.

- [x] **N135. Extract a shared clipboard-copy helper and fix CharacterKeysPanel's swallowed copy failure.** ✅ DONE 2026-07-29 — commit e076e4bb
  Confirmed: `EntityPanel.tsx`'s `CopyLinkButton` and `BuildReportPanel.tsx`'s `onCopy` each wrapped `navigator.clipboard.writeText` in their own try/catch + `toast.error`, but `CharacterKeysPanel.copyKey` had no `.catch()` at all — a denied clipboard silently did nothing (toast was already imported there but unused for this path). Added `src/lib/clipboard.ts` exporting `copyToClipboard(text)`, which resolves `false` (and logs via `logger.warn`) instead of throwing on failure. Routed all three call sites through it — `EntityPanel`'s handler dropped its local try/catch + `logger` import (now unused there) in favor of checking the boolean; `BuildReportPanel.onCopy` and `CharacterKeysPanel.copyKey` likewise branch on the boolean, each keeping their own existing toast message text. `CharacterKeysPanel.copyKey` now shows `toast.error("Could not copy key")` on failure instead of silently doing nothing.
  **Tests:** new `src/test/lib/clipboard.test.ts` (3 tests) — resolves true and writes the given text on success; resolves false instead of throwing when the write rejects; resolves false instead of throwing when `navigator.clipboard` is unavailable. Plus 1 new test in `src/test/secrets/CharacterKeysPanel.test.tsx` — a rejected clipboard write shows the error toast and leaves the copied-indicator unset.
  **Gate:** typecheck clean · lint 0 errors (18 pre-existing, unchanged) · 2949 tests green across the 4 shards (737+640+866+706 — +4 over the N134 baseline of 2945, matching the 4 new tests). One shard hit the documented `onTaskUpdate` RPC flake (0 real failures, not re-run per policy). Pure client-side helper + 3 call-site edits + test change — no `scripts/`-only edit, no fog/soundscape touch, no shipped-artifact shape change — so `atlas:publish` wasn't required.

- [x] **N136. Deduplicate the four copy-pasted safe-localStorage guards into one helper.** ✅ DONE 2026-07-30 — commit 9c73f80f
  Confirmed: the identical private-browsing-safe `getStorage()` accessor was byte-for-byte copy-pasted in four player-local stores — `playerNotes.ts`, `visitedPlaces.ts`, `soundPrefs.ts`, and `playerSecretsStore.ts` (the "one more"). Extracted `src/lib/safeStorage.ts` exporting `getStorage()` and routed all four through it; behavior identical, no store-level test needed to change.
  **Tests:** new `src/test/lib/safeStorage.test.ts` (3 tests) — returns `window.localStorage` when writable; returns `null` when the probe `setItem` throws (mocks `Storage.prototype.setItem`, mirroring the existing `playerSecretsStore.test.ts` pattern); cleans up its own probe key on success.
  **Gate:** typecheck clean · lint 0 errors (18 pre-existing, unchanged) · 2952 tests green across the 4 shards (737+640+852+723 — +3 over the N135 baseline of 2949, matching the 3 new tests; shard boundaries shifted slightly because a new test file was added). One shard hit the documented `onTaskUpdate` RPC flake (0 real failures, not re-run per policy). Pure client-side helper + 4 call-site edits + new test file — no `scripts/`-only edit, no fog/soundscape touch, no shipped-artifact shape change — so `atlas:publish` wasn't required.
  **Side-effect (recurring, same as N107–N135):** the sharded vitest run's build-pipeline tests deleted the same 6 content-hashed audio files under `public/atlas/assets/audio/` in the run worktree. Caught via `git status` before the commit; reverted with `git checkout --` — no unrelated artifact churn landed on `auto/continuous-dev`.

- [x] **N137. Add unit tests for the service-worker enable/disable gate in `src/pwa.ts`.** ✅ DONE 2026-07-30 — commit 6394ebcb
  Confirmed: `shouldEnableServiceWorker` and its private helpers `isInIframe`/`isPreviewHost` (`src/pwa.ts:20-45`) had zero test coverage. Both helpers are unexported, so added `src/test/pwa.test.ts` (new file, 11 tests) exercising every branch through the composed `shouldEnableServiceWorker()` decision: no SW support, dev mode, iframe (including the cross-origin `window.top` access throwing), each of the five `isPreviewHost` substring/suffix branches, a plain `lovable.app` host without "preview", and the fully-enabled happy path. No source change needed.
  **Test mechanics:** `navigator.serviceWorker` toggled via `defineProperty`/`delete` (the gate uses `"serviceWorker" in navigator`, so `delete` is required — setting `value: undefined` still leaves the property present); `window.location` replaced wholesale per test (jsdom's real `Location.hostname` is a non-configurable getter, so `defineProperty` on it directly throws — swapping the whole `window.location` object sidesteps that); `window.top` swapped via `defineProperty` (plain object for "iframed", a throwing getter for the cross-origin case); `import.meta.env.PROD` via `vi.stubEnv`, mirroring the existing `atlas-dm-tools-gate.test.tsx` pattern.
  **Gate:** typecheck clean (both `typecheck` + `typecheck:scripts`) · lint 0 errors (18 pre-existing, unchanged) · 2963 tests green across the 4 shards (737+640+859+727 — +11 over the N136 baseline of 2952, matching the 11 new tests). One shard hit the documented `onTaskUpdate` RPC flake (0 real failures, not re-run per policy). Pure new test file, no source change — no `scripts/`-only edit, no fog/soundscape touch, no shipped-artifact shape change — so `atlas:publish` wasn't required.
  **Side-effect (recurring, same as N107–N136):** the sharded vitest run's build-pipeline tests deleted the same 6 content-hashed audio files under `public/atlas/assets/audio/` in the run worktree. Caught via `git status` before the commit; reverted with `git checkout --` — no unrelated artifact churn landed on `auto/continuous-dev`.

- [x] **N138. Fix two stale/fictional docs claims (import audit + import-batch rollback).** ✅ DONE 2026-07-30 — commit ae2a0562
  Confirmed: `docs/IMPORT_EXPORT.md:180` said the asset audit was "advisory / not chained into atlas:publish," but `publish-orchestrator.ts` runs `auditAssets` as its `audit-assets` step, and `atlas:publish` calls `atlas:scan` (`package.json:24-25`) — so it does run on every publish. `docs/WORKFLOWS.md:102-105` described an import-batch record + "remove this batch" rollback affordance as shipped, but `IMPORT_EXPORT.md:67-89`'s own "Import-batch tracking and undo (planned)" section marks that feature not yet built — the current wizard produces a YAML patch for manual paste/commit only.
  **Fix:** reworded `IMPORT_EXPORT.md:180` to state the audit is chained into `atlas:publish` via `atlas:scan`, and note `npm run atlas:audit-assets` for a standalone check. Reworded `WORKFLOWS.md`'s step 4 to describe the actual YAML-paste-and-commit workflow, with `git log` as the audit trail, and to point at `IMPORT_EXPORT.md` for the still-planned batch-rollback affordance instead of describing it as already shipped.
  **Gate:** typecheck clean (both `typecheck` + `typecheck:scripts`) · lint 0 errors (18 pre-existing, unchanged) · 2963 tests green across the 4 shards (737+640+859+727 — unchanged from the N137 baseline, as expected for a docs-only change with no test impact). One shard hit the documented `onTaskUpdate` RPC flake (0 real failures, not re-run per policy). Docs-only edit — no `scripts/`-only edit, no fog/soundscape touch, no shipped-artifact shape change — so `atlas:publish` wasn't required.
  **Side-effect (recurring, same as N107–N137):** the sharded vitest run's build-pipeline tests deleted the same 6 content-hashed audio files under `public/atlas/assets/audio/` in the run worktree. Caught via `git status` before the commit; reverted with `git checkout --` — no unrelated artifact churn landed on `auto/continuous-dev`.

- [x] **N139. Wire the publish workflow's `changelog` input into the run summary.** ✅ DONE 2026-07-30 — commit 0b2564b6
  Confirmed: `.github/workflows/publish-atlas.yml` defines a `workflow_dispatch` `changelog` input (`:20-25`) and its header comment advertises it (`:5`), but nothing in the job steps consumed it — a dispatched publish silently dropped the note.
  **Fix:** added a "Record changelog note" step (first in the `build` job, right after checkout), gated on `github.event_name == 'workflow_dispatch' && inputs.changelog != ''`, that appends a `### Changelog note` heading plus the note text to `$GITHUB_STEP_SUMMARY`. The input is passed through an `env:` var (`CHANGELOG_NOTE`) rather than spliced directly into the `run:` script, avoiding the classic GitHub Actions shell-injection anti-pattern for untrusted/expression input. Empty input is a no-op (`if:` condition short-circuits, step doesn't run).
  **Gate:** lint 0 errors (18 pre-existing, unchanged) · typecheck clean (both `typecheck` + `typecheck:scripts`) · 2963 tests green across the 4 shards (737+640+859+727 — unchanged from the N138 baseline, as expected: workflow-YAML-only change, no test files touched). One shard hit the documented `onTaskUpdate` RPC flake (0 real failures, not re-run per policy). CI-workflow-only edit, no app source touched — `atlas:publish` wasn't required.
  **Side-effect (recurring, same as N107–N138):** the sharded vitest run's build-pipeline tests deleted the same 6 content-hashed audio files under `public/atlas/assets/audio/` in the run worktree. Caught via `git status` before the commit; reverted with `git checkout --` — no unrelated artifact churn landed on `auto/continuous-dev`.

- [x] **N140. Add `bin/` launchers for the three missing safety scanners.** ✅ DONE 2026-07-30 — commit e3614d60
  Confirmed (with a small correction): `bin/` had launchers for `check-secrets`, `check-derived`, `check-shape`, and `audit-assets`, but not for `check-image-privacy`, `check-fog`, or `check-player-secrets`. Of those three, `check-image-privacy` and `check-fog` already had `npm run atlas:check-*` scripts (just no launcher); `check-player-secrets` (a distinct scanner from `check-secrets`, wired into `publish-orchestrator.ts` but never exposed as its own npm script) had neither.
  **Fix:** added `"atlas:check-player-secrets": "tsx scripts/check-player-secrets.ts"` to `package.json` (mirroring `atlas:check-secrets`), then added all three `.cmd` launchers (`atlas-check-image-privacy.cmd`, `atlas-check-fog.cmd`, `atlas-check-player-secrets.cmd`) mirroring the existing launcher template (`pushd`/`npm run <name> dist`/`pause`/exit code echo). Documented all three in `COMMANDS.md` alongside the existing scanner rows.
  **Smoke test:** ran `atlas-check-player-secrets.cmd` end-to-end via `cmd /c` with piped stdin (to satisfy the interactive `pause`) — exited 0 against `dist`.
  **Commits:** `e3614d60` (fix) on `run/n140-scanner-launchers`, fast-forwarded into `auto/continuous-dev`. The run branch was deleted locally after the ff-only merge was pushed (its commit is safely reachable from `auto/continuous-dev`'s pushed tip — no unique work at risk, so no tag-and-push was needed per the safe-cleanup protocol's own logic).
  **Gate:** typecheck clean (both `typecheck` + `typecheck:scripts`) · lint 0 errors (18 pre-existing, unchanged) · 2963 tests green across the 4 sequential shards (737+640+859+727 — unchanged from the N139 baseline, as expected: no test files touched). One shard hit the documented `onTaskUpdate` RPC flake (0 real failures, not re-run per policy). `package.json` + `bin/` + docs only — no `scripts/`-only edit, no fog/soundscape touch, no shipped-artifact shape change — so `atlas:publish` wasn't required.
  **Pre-commit hook note (new, worth flagging for a future run):** the hook's own unsharded `vitest run --changed` hit a real `× ... failed` in `src/test/rumor-semantics.test.ts` ("expected 1 to be +0") — but the hook's flake-detection `grep -qE "failed \|"` (meant to distinguish real failures from the `onTaskUpdate` RPC-timeout noise) did **not** match, because vitest's colorized summary line interposes ANSI escape codes between "failed" and "|" (`1 failed[39m[22m[2m | [22m...`), so the literal substring "failed |" never appears contiguously. The hook exited 0 and let the commit through despite a real test-run failure. Re-ran `rumor-semantics.test.ts` alone afterward — all 8 tests passed, confirming the failure was resource-contention flakiness from the unsharded run (consistent with the documented whole-suite-OOM risk), not a regression from this unit's change (which touched only `package.json`, new `.cmd` files, and docs — no code path shared with the build pipeline). No action taken this run since diagnosing the hook's grep pattern and choosing a robust fix is its own small unit, not a `bin/`-launcher change — flagging as a candidate nice-to-have for a future run.
