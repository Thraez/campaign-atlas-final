...
- [ ] **N103. The DM editor's "World name" setting is a dead end — it never reaches world.yaml/atlas.json.**
  ...
  - **Done when:** editing the World name marks world.yaml dirty and Save writes the new `name:` into `world.yaml`; reload shows it persisted; a test covers the dirty+serialize path.
  - **Gate:** standard gate.
  ~2 runs.
...
- [ ] **N109. Render inline markdown (bold/italic/links) inside callout titles.**
  ...
  - **Done when:** bold/italic/links/code in a callout title render as formatted HTML; escaping is preserved; a test covers an inline-formatted title.
  - **Gate:** standard gate.
  ~1 run.

- [ ] **N110. Multi-line footnote definitions corrupt into a stray code block.**
  `FN_DEF_RE` is single-line (`/^\[\^([^\]]+)\]: *(.*)/`, `src/atlas/content/markdownCore.ts:120`) and the tokenizer consumes only the first physical line (`:130-142`), so an indented continuation line of a `[^id]:` definition is left to `marked`, which sees the indentation as a code block. Consume continuation lines (indented or lazy) into the footnote definition.
  - **Done when:** a footnote definition with a wrapped/continued line renders as one footnote with no stray code block; single-line footnotes are unchanged; a test covers the multi-line case.
  - **Gate:** standard gate.
  ~2 runs.

- [ ] **N111. Colliding, unstyled footnotes in the printable handout.**
  The footnote extension emits ids scoped only by the footnote's own label (`id="fnref-{id}"` / `href="#fn-{id}"`, `src/atlas/content/markdownCore.ts:165-171`), so printing a multi-entity handout bundle cross-wires footnotes between entities that share a label, and single-entity handouts render footnotes with no styling. Scope footnote ids per entity in the print path and add minimal print styling.
  - **Done when:** a multi-entity handout keeps each entity's footnotes self-contained, and footnotes render with visible styling; a test covers id-scoping across two entities.
  - **Gate:** standard gate.
  ~2 runs.

- [ ] **N112. Honor Obsidian's image-resize pipe syntax instead of reading it as alt text.**
  `resolveImageEmbeds` treats everything after `|` in an embed as alt text (`src/atlas/content/renderEntityMarkdown.ts:21-26`), so `![[image.png|300]]` → `![300](path)` and `![[image.png|300x200]]` → `![300x200](path)` instead of setting width/height (which `sanitizeHtml.ts:91-92` already allows). Parse a trailing `|W` / `|WxH` as dimensions; keep non-numeric pipe content as alt text. (Coordinate with Q51, which also edits this function.)
  - **Done when:** `![[img.png|300]]` renders an `<img width="300">` and `|300x200` sets both dimensions; `|caption` still becomes alt text; tests cover each.
  - **Gate:** standard gate.
  ~1 run.

- [ ] **N113. Ruler clicks also open the region underneath them.**
  Region polygons wire `click` → `onOpenEntity` with no `stopPropagation` (`src/pages/AtlasViewer.tsx:892-906`), so while the ruler tool is active, clicking to drop a measurement point on top of a region also opens that region's entity. Suppress the region click while the ruler is active (or stop propagation from the ruler layer).
  - **Done when:** placing ruler points over a region measures without opening the region; normal region clicks (ruler off) still open the entity; a test covers the active-ruler case.
  - **Gate:** standard gate.
  ~1 run.

- [ ] **N114. Reset the distance ruler when the active map changes.**
  `RulerLayer` is mounted without a `key` (`src/pages/AtlasViewer.tsx:660-666`) and only clears `points` on the active→inactive transition (`RulerLayer.tsx:26-34`), so switching maps with a measurement in progress leaves stale points floating at meaningless coordinates on the new map. Reset on map switch (e.g. `key={activeMap.id}` or clear on map change). (Distinct from Q7's third-click reset.)
  - **Done when:** changing the active map clears any in-progress ruler measurement; a test covers the reset.
  - **Gate:** standard gate.
  ~1 run.
...
- [ ] **N116. Ruler point mis-placement on wrap-around (`wrapX`) maps.**
  ...
  - **Done when:** on a `wrapX` map, ruler points land where clicked on any wrapped copy and the measured distance is correct; a test covers a wrapped click.
  - **Gate:** standard gate.
  ~1 run.

- [ ] **N117. Inverted horizontal grid lines on non-exact-multiple map heights.**
  The square-grid branch builds horizontals as `[y, 0]`/`[y, map.width]` using the raw image row `y` as the Leaflet lat (`src/atlas/map/geometry.ts:56-61`), skipping the `map.height − y` flip that every other conversion uses (the hex branch flips correctly at `:79`). For heights that aren't an exact multiple of the grid size the partial line lands on the wrong edge, out of step with markers/regions. Apply the flip.
  - **Done when:** square-grid horizontals align with the flat-CRS convention on non-multiple heights; exact-multiple grids are visually unchanged; a test covers a non-multiple height.
  - **Gate:** standard gate.
  ~1 run.

- [ ] **N118. Exact-phrase search silently drops tag-only matches.**
  `matchesPhrases` builds its haystack from title/aliases/summary/body but omits `e.tags` (`src/atlas/search/parseSearchQuery.ts:43-50`), so a quoted `"phrase"` that appears only in an entry's tags won't match even though unquoted search does consider tags — an inconsistency in the shipped phrase search (E5). Include tags in the phrase haystack.
  - **Done when:** a quoted phrase matching only a tag returns the entry; a test covers a tag-only phrase hit.
  - **Gate:** standard gate.
  ~1 run.

- [ ] **N119. Browse's "all" count is backwards and type-chip counts ignore the active query.**
  On `AtlasBrowse.tsx`, the "all" chip shows `entries.length` (`:154`) but `entries` already has `activeType` applied (`:36`), so with a type selected "all" echoes the filtered count instead of the total; and the per-type chip counts don't track the text query. Compute the "all" count from the pre-type list and make type counts reflect the current query.
  - **Done when:** "all" shows the true unfiltered total, each type chip shows its count under the active query, and selecting a type doesn't change the "all" number; tests cover the counts.
  - **Gate:** standard gate.
  ~1 run.

- [ ] **N120. Reset Browse's text/type filters when navigating between facets.**
  `AtlasBrowse` holds `query` and `activeType` in local state with nothing resetting them on route-facet change (`:22-23`), and `/atlas/browse`, `/atlas/tag/:tag`, `/atlas/type/:type` all render the same component, so a filter typed on one facet silently persists (and can zero out results) after navigating to another. Reset the secondary filters when the facet changes. (Coordinate with Q21 if URL-persistence lands.)
  - **Done when:** navigating between browse/tag/type facets clears the text + type filters; a test covers the reset.
  - **Gate:** standard gate.
  ~1 run.

- [ ] **N121. Browse's tag/type empty state blames the facet instead of the filter text.**
  The zero-results message is chosen purely from `mode` (`AtlasBrowse.tsx:174-189`), not from *why* `entries` is empty — but `entries` is narrowed by the facet AND the secondary type/query filters (`:31-48`), so a tag page with results hidden only by an active text filter still says the tag itself is empty. Distinguish "nothing under this facet" from "your filter hid everything."
  - **Done when:** an empty result caused by the secondary filter shows a "clear filters" message, not a "nothing here" one; a test covers both causes.
  - **Gate:** standard gate.
  ~1 run.

- [ ] **N122. Search results keep their old scroll offset when filters change.**
  Changing `query`/`activeType`/`activeTag`/`thisMapOnly`/`recentOnly` resets `activeIndex` to `-1`, but the `scrollIntoView` effect is gated on `activeIndex >= 0` (`src/atlas/search/SearchPalette.tsx:147-159`), so the results list stays scrolled to the previous position after a filter change. Scroll the list back to top when the filter set changes.
  - **Done when:** changing any filter scrolls the results list to the top; keyboard navigation still scrolls the active row into view; a test covers the reset.
  - **Gate:** standard gate.
  ~1 run.

- [ ] **N123. The hover-peek card shows the raw internal type slug, not the player label.**
  `HoverPeekCard` renders `{entity.type}` directly as the chip text (`src/atlas/peek/HoverPeekCard.tsx:43-45`), leaking internal slugs (e.g. `npc-major`) where the reading panel already uses `playerTypeLabel` (`src/atlas/content/typeLabel.ts:17-28`). Use the canonical label. (Distinct from Q93's broken-image fallback.)
  - **Done when:** the peek chip shows the player-facing label via `playerTypeLabel`; a test covers a slug→label case.
  - **Gate:** standard gate.
  ~1 run.
...
- [ ] **N126. The "already exists" import-conflict toast tells the DM to click a button that can't fix that row.**
  ...
  - **Done when:** the already-exists conflict message names a step that resolves that row; a test covers the guidance for the on-disk-only conflict.
  - **Gate:** standard gate.
  ~1 run.

- [ ] **N127. Show image credit/attribution in the printable handout.**
  The reading panel resolves and shows an image credit for every hero/gallery thumbnail via `resolveImageCredit()` + `CreditBadge` (`src/atlas/entity/EntityPanel.tsx:52-62,392-408`), but `printHandout` ships entity images with no attribution. Carry the same credit into the handout so printed images keep their required attribution.
  - **Done when:** a printed handout shows the image credit wherever the live panel would; a test covers an image with a `credit`.
  - **Gate:** standard gate.
  ~1 run.
...
- [ ] **N129. Let Escape close the desktop entity reading panel.**
  ...
  - **Done when:** with the desktop entity panel open, Escape closes it (after peek/search are already dismissed); a test covers the handler order.
  - **Gate:** standard gate.
  ~1 run.

- [ ] **N130. Give the DM editor's Cmd/Ctrl+K command palette dialog + listbox semantics.**
  The editor's own command palette (`src/atlas/shell/CommandPalette.tsx`, separate from the player search palette) has no ARIA structure: the overlay has no `role="dialog"`/`aria-modal` (`:34-42`) and results are a plain `<ul>/<li>/<button>` with no `role="listbox"`/`role="option"`/`aria-activedescendant` (`:61-77`), so screen-reader users get no dialog boundary or active-option announcement. Add dialog + combobox/listbox semantics.
  - **Done when:** the palette exposes a labeled dialog and a listbox whose active option is announced as the DM arrows through; a test covers the roles.
  - **Gate:** standard gate.
  ~2 runs.

- [ ] **N131. Move focus into rail panels on open and restore it to the trigger on close.**
  `EditorPanelHost` is the shared host every rail panel mounts into (World, Maps, Assets, Entities, Regions, Routes, Fog, Soundscape, Layers) and does no focus management (`src/atlas/shell/EditorPanelHost.tsx:34-41,64-107`): opening a panel leaves focus on the map, and closing (incl. Escape) doesn't return focus to the rail button. Move focus into the panel on open and restore it on close.
  - **Done when:** opening a rail panel focuses its first control; closing returns focus to the triggering rail button; a test covers open+close focus.
  - **Gate:** standard gate.
  ~2 runs.
