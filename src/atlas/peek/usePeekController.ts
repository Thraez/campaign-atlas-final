...
export function usePeekController({ pointerFine }: { pointerFine: boolean }) {
  ...
  const show = useCallback((entityId: string, rect: DOMRect) => {
    const position = computePeekPosition(
      rect,
  ...
  const scheduleClose = useCallback(() => {
    ...
    closeTimer.current = window.setTimeout(() => {
      setPeek(null);
      coolingUntil.current = Date.now() + COOLING;
    }, CLOSE_GRACE);
  }, []);

  const onTriggerEnter = useCallback(
    (entityId: string, rect: DOMRect, start?: { x: number; y: number }) => {
  ...
  const tapPeek = useCallback(
    (entityId: string, rect: DOMRect): string => {
      if (peek?.entityId === entityId) {
        setPeek(null);
        return entityId;
      }
      show(entityId, rect);
