...
export function useRegionDraft(
  ...
  const patch = useCallback(
    (id: string, partial: Partial<Region>) => {
      mutateDraft((d) => {
  ...
  const getEffective = useCallback(
    (id: string): Region | null => effective.find((r) => r.id === id) ?? null,
    [effective],
  );
    ...
    for (const r of effective) {
      ...
      if (r.points.length < 3)
        out.push({
          ...
          message: `Region "${r.name}" needs ≥ 3 points`,
          regionId: r.id,
        });
      if (map && r.mapId !== map.id)
        out.push({
          severity: "warning",
          code: "region-wrong-map",
          message: `Region "${r.name}" mapId "${r.mapId}" doesn't match active map`,
          regionId: r.id,
        });
      if (opts.entityIds && r.entityId && !opts.entityIds.has(r.entityId)) {
        out.push({
          severity: "warning",
          code: "region-unknown-entity",
          message: `Region "${r.name}" links to unknown entity "${r.entityId}"`,
          regionId: r.id,
        });
      }
      if (
        ...
        r.visibility === "player" &&
        r.entityId &&
        opts.dmEntityIds.has(r.entityId)
      ) {
        out.push({
          severity: "blocking",
          code: "spoiler-leak",
          message: `Player-visible region "${r.name}" links to DM-only entity`,
          regionId: r.id,
        });
      }
      if (map) {
        ...
        if (oob)
          out.push({
            ...
            message: `Region "${r.name}" has points outside map bounds`,
            regionId: r.id,
          });
      }
      if (r.fillOpacity != null && (r.fillOpacity < 0 || r.fillOpacity > 1)) {
        out.push({
          severity: "warning",
          code: "region-opacity",
          message: `Region "${r.name}" fillOpacity must be 0–1`,
          regionId: r.id,
        });
      }
      if (r.strokeOpacity != null && (r.strokeOpacity < 0 || r.strokeOpacity > 1)) {
        out.push({
          severity: "warning",
          code: "region-opacity",
          message: `Region "${r.name}" strokeOpacity must be 0–1`,
          regionId: r.id,
        });
      }
    }
