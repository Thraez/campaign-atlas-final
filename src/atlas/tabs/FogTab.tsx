...
export function FogTab({
  ...
  const yamlBlock = useMemo(
    () => dumpYaml({ maps: [{ id: map.id, fog: fogToYamlObject(fog) }] }),
    [fog, map.id],
  );
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between rounded-md border border-border p-2">
          <div>
            ...
            <div className="text-[10px] text-muted-foreground">
              Players see the map covered except inside reveals.
            </div>
          </div>
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <Button
            ...
            onClick={() => setShowFogPreview(!showFogPreview)}
          >
        ...
        <div className="rounded-md border border-border p-2 space-y-2 bg-card/50">
          ...
          {tool === "polygon" && (
            <div className="space-y-1">
              <p className="text-[10px] text-muted-foreground italic">
                Click on the map. Enter finishes, Esc cancels, Backspace removes last point. (
          ...
          {tool === "circle" && (
            <div className="space-y-1">
              <p className="text-[10px] text-muted-foreground italic">
                Click a center on the map. ({draftPoints.length === 0 ? "no anchor" : "anchor set"})
              </p>
              <div className="grid grid-cols-2 gap-1 items-end">
                <div>
                  ...
                  <Input
                    type="number"
                    value={circleRadius}
                    onChange={(e) => setCircleRadius(Number(e.target.value) || 0)}
                    className="h-6 text-xs"
                  />
                </div>
                <div className="flex gap-1">
          ...
          <div className="pt-1 border-t border-border space-y-1.5">
            ...
            {regionApi?.selectedId ? (
              <Button
                ...
                onClick={() => {
                  ...
                  if (r) {
                    revealRegion(r);
                    toast.success(`Revealed region "${r.name}"`);
                  }
                }}
              >
              ...
              <p className="text-[10px] text-muted-foreground italic">
                Select a region in the Regions tab to reveal it.
              </p>
            )}
            ...
            {routeApi?.selectedId ? (
              <div className="flex gap-1">
                <Input
                  type="number"
                  value={routePadding}
                  onChange={(e) => setRoutePadding(Number(e.target.value) || 0)}
              ...
              <p className="text-[10px] text-muted-foreground italic">
                Select a route in the Routes tab to reveal around it.
              </p>
            )}
            ...
            <div className="flex gap-1 items-end">
              ...
              <div>
                ...
                <Input
                  type="number"
                  value={pinRadius}
                  onChange={(e) => setPinRadius(Number(e.target.value) || 0)}
                  className="h-7 text-xs w-20"
                />
              </div>
              <Select onValueChange={(v) => setPinId(v)}>
                ...
                <SelectContent>
                  {Array.from(placementsByEntity.keys()).map((eid) => (
                    <SelectItem key={eid} value={eid} className="text-xs">
                      {project.entities.find((e) => e.id === eid)?.title ?? eid}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
            ...
            <Button
              ...
              onClick={() => {
                ...
                if (!c) {
                  toast.error("Entity has no placement on this map.");
                  return;
                }
        ...
        <div className="rounded-md border border-border p-2 space-y-2 bg-card/50">
          ...
          {tool === "fog-polygon" && (
            <div className="space-y-1">
              <p className="text-[10px] text-muted-foreground italic">
                Click on the map to add fog. Enter finishes, Esc cancels. ({draftPoints.length} pts)
              </p>
              <div className="flex gap-1">
                <Button
                  size="sm"
          ...
          <div>
            <Label htmlFor="fog-feather-px" className="text-[10px]">
              Soft edge (px)
            </Label>
            <Input
              id="fog-feather-px"
        ...
        <div className="space-y-1">
          ...
          {fog.reveals.length === 0 && (
            <p className="text-[10px] text-muted-foreground italic">
              No reveals yet — the entire map is covered.
            </p>
          )}
          {fog.reveals.map((poly, i) => (
            <div
              key={i}
              className="flex items-center gap-1 px-2 py-1 rounded border border-border bg-card/30"
            >
              <span className="text-[10px] flex-1">
                Reveal #{i + 1} · {poly.length} pts
              </span>
              <Button
                size="sm"
                variant="ghost"
                className="h-5 w-5 p-0 text-destructive"
                onClick={() => removeReveal(i)}
              >
        ...
        {(fog.conceals?.length ?? 0) > 0 && (
          <div className="space-y-1">
            ...
            {fog.conceals!.map((poly, i) => (
              ...
              >
                <span className="text-[10px] flex-1">
                  Fog #{i + 1} · {poly.length} pts
                </span>
                <Button
                  size="sm"
                  variant="ghost"
                  className="h-5 w-5 p-0 text-destructive"
                  onClick={() => removeConceal(i)}
                >
                  <Trash2 className="h-3 w-3" />
                </Button>
              </div>
