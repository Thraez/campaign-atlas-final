...
function makeStubDeps() {
  ...
  const mockCtx: any = {
    ...
    suspend: vi.fn(async () => { mockCtx.state = "suspended"; }),
    close: vi.fn(async () => {}),
  };
  return {
    deps: {
...
function MuteProbe() {
  ...
  return (
    <button onClick={() => setMuted(!muted)}>{muted ? "muted" : "unmuted"}</button>
  );
}
...
describe("SoundSettingsProvider", () => {
  ...
  it("starts from defaults and toggles calm mode, persisting + reflecting on <html>", () => {
    render(<SoundSettingsProvider><CalmProbe /></SoundSettingsProvider>);
  ...
  it("enableSound sets soundEnabled: true and persists it", async () => {
    ...
    render(<SoundSettingsProvider deps={deps}><EnableProbe /></SoundSettingsProvider>);
  ...
  it("setMuted(true/false) updates the muted value in the exposed context", () => {
    const { deps } = makeStubDeps();
    render(<SoundSettingsProvider deps={deps}><MuteProbe /></SoundSettingsProvider>);
  ...
  it("muted state passes through to engine.setMuted on toggle", () => {
    ...
    render(<SoundSettingsProvider deps={deps}><EngineCapture /></SoundSettingsProvider>);
