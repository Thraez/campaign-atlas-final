...
describe("check-player-secrets", () => {
  it("passes on clean ciphertext-only secrets", () => {
    writeAtlas([{
      id: "a", body: "normal text",
      secrets: [{ id: "s", lockType: "password", teaser: "hint", salt: "QUJD", iv: "QUJD", ciphertext: "QUJDREVG" }],
    }]);
  ...
  it("fails if a secret has a 'reveal' field", () => {
    writeAtlas([{
      id: "b", body: "x",
      secrets: [{ id: "s", lockType: "password", reveal: "plaintext", salt: "Q", iv: "Q", ciphertext: "Q" }],
    }]);
  ...
  it("fails if a secret has a 'password' field", () => {
    writeAtlas([{
      id: "c", body: "x",
      secrets: [{ id: "s", lockType: "password", password: "pw", salt: "Q", iv: "Q", ciphertext: "Q" }],
    }]);
    expect(run({ dir })).toBe(13);
  });
