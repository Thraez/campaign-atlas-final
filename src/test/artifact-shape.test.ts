...
let tmpRoot: string;
beforeAll(() => {
  tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "atlas-shape-"));
});
afterAll(() => {
...
describe("scanArtifactShape — soundscape assertions", () => {
  ...
  it("flags a DM-visibility area", () => {
    ...
    const r = scanArtifactShape(atlas);
    expect(
      r.violations.some((v) => v.field.includes("soundscape") && v.message.includes("visibility")),
    ).toBe(true);
  });
  ...
  it("flags an area with a non-neutralised id", () => {
    ...
    const r = scanArtifactShape(atlas);
    expect(
      r.violations.some((v) => v.field.includes("soundscape") && v.message.includes("id")),
    ).toBe(true);
  });
