...
describe("no-loss invariant — per holder", () => {
  it("region draft survives serialize → deserialize → fresh hook applySnapshot", () => {
    ...
    act(() => {
      fresh.result.current.applySnapshot(restored.regionByMap.A);
    });
  ...
  it("route draft survives the round-trip", () => {
    ...
    act(() => {
      ...
      result.current.addDraftPoint([10, 10]);
    });
    act(() => {
      result.current.finishDraw();
    });
    ...
    act(() => {
      fresh.result.current.applySnapshot(restored.routeByMap.A);
    });
    expect(fresh.result.current.snapshot()).toEqual(snap);
  });
  ...
  it("fog draft survives the round-trip", () => {
    ...
    act(() => {
      result.current.addDraftPoint([0, 0]);
      result.current.addDraftPoint([9, 0]);
      result.current.addDraftPoint([5, 9]);
    });
    act(() => {
      result.current.finishDraftPolygon();
    });
    const snap = result.current.snapshot();
    ...
    act(() => {
      fresh.result.current.applySnapshot(restored.fogByMap.A);
    });
    expect(fresh.result.current.snapshot()).toEqual(snap);
  });
  ...
  it("switch-away-and-back is non-destructive (per-map slices)", () => {
    const onA = renderHook(({ m }) => useRegionDraft(m, {}, undefined), {
      initialProps: { m: mapA },
    });
    act(() => {
      onA.result.current.startDraw();
    });
    act(() => {
    ...
    onA.rerender({ m: mapB });
    act(() => {
      onA.result.current.applySnapshot(
        state.regionByMap.B ?? { edits: {}, added: [], deleted: [] },
      );
    });
    ...
    onA.rerender({ m: mapA });
    act(() => {
      onA.result.current.applySnapshot(state.regionByMap.A);
    });
