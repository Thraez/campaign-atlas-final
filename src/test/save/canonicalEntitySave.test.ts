...
const base: Entity = {
  ...
  relationships: [{ targetId: "villain", label: "rivals", visibility: "dm" }],
  body: "",
  bodyHtml: "",
  links: [],
  tags: [],
...
describe("entityFrontmatterPatches", () => {
  ...
  it("preserves non-empty relationships from the draft", () => {
    const rel = { targetId: "ally", label: "allied_with", visibility: "player" as const };
  ...
  it("falls back to entity relationships when draft has none; strips if entity list is empty", () => {
    ...
    const emptyBase = { ...base, relationships: [] } as unknown as Entity;
