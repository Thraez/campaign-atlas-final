...
describe("computeAtlasDiff", () => {
  ...
  it("detects visibility changes", () => {
    const before = project(
      [entity({ id: "a", title: "A", visibility: "rumor" })],
      [map({ id: "m" })],
    );
    const after = project(
      [entity({ id: "a", title: "A", visibility: "player" })],
      [map({ id: "m" })],
    );
    const diff = computeAtlasDiff(before, after);
  ...
  it("detects body-substance changes via signature", () => {
    const before = project(
      [entity({ id: "a", title: "A", body: "short body here" })],
      [map({ id: "m" })],
    );
    const after = project(
      [entity({ id: "a", title: "A", body: "very different body content now" })],
      [map({ id: "m" })],
    );
    const diff = computeAtlasDiff(before, after);
  ...
  it("ignores trivial whitespace-only body edits", () => {
    const before = project(
      [entity({ id: "a", title: "A", body: "exact same body content" })],
      [map({ id: "m" })],
    );
    const after = project(
      [entity({ id: "a", title: "A", body: "  exact same body content  " })],
  ...
  it("detects region added on the same map", () => {
    ...
    const diff = computeAtlasDiff(before, after);
    expect(
      diff.overlays.find((o) => o.kind === "region-added" && o.name === "New region"),
    ).toBeDefined();
  });
  ...
  it("detects entity summary changes", () => {
    const before = project(
      [entity({ id: "a", title: "A", summary: "original" })],
      [map({ id: "m" })],
    );
    const after = project(
      [entity({ id: "a", title: "A", summary: "updated" })],
      [map({ id: "m" })],
    );
    const diff = computeAtlasDiff(before, after);
  ...
  it("detects route added and removed on the same map", () => {
    ...
    const diff = computeAtlasDiff(before, after);
    expect(
      diff.overlays.find((o) => o.kind === "route-added" && o.name === "New Road"),
    ).toBeDefined();
    expect(
      diff.overlays.find((o) => o.kind === "route-removed" && o.name === "Old Road"),
    ).toBeDefined();
  });
  ...
  it("detects region removed on the same map", () => {
    ...
    const diff = computeAtlasDiff(before, after);
    expect(
      diff.overlays.find((o) => o.kind === "region-removed" && o.name === "Gone region"),
    ).toBeDefined();
  });
  ...
  it("emits overlay removals for regions and routes on a removed map", () => {
    ...
    expect(
      diff.overlays.find((o) => o.kind === "region-removed" && o.name === "Lost region"),
    ).toBeDefined();
    expect(
      diff.overlays.find((o) => o.kind === "route-removed" && o.name === "Lost road"),
    ).toBeDefined();
  });
